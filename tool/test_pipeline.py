...
class basic_modules.tool.Tool(configuration = {})
	...
	each method should be decorated using the @task decorator. Task constraints can be configured using
...
class testToolPablo(Tool):
	...
	def __init__(self, configuration = none):
		...
		""" utils.logger.debug(message, *args, **kargs) 
			message is the message format string, and args are the arguments which are merged into the msg using the 
